    ...
    <div>
      ...
      <div className="max-w-md mx-auto mb-6">
        <div className="relative">
          ...
          <input type="text" placeholder="记账、打卡、Todo..."
            value={search}
            onChange={e => setSearch(e.target.value)}
            className="w-full bg-[var(--color-surface)] rounded-[var(--radius-lg)] pl-10 pr-4 py-2.5 text-[14px] text-[var(--color-text-primary)] placeholder-[var(--color-text-muted)] border border-[var(--color-border)] focus:border-[var(--color-accent)] focus:ring-0 outline-none transition-ui" />
        </div>
      </div>
      ...
      <div className="flex justify-center gap-1.5 mb-10">
        {CATEGORIES.map(cat => (
          <button key={cat}
            ...
            className={"px-4 py-1.5 rounded-[var(--radius-sm)] text-[13px] font-medium transition-ui " +
              (activeCategory === cat
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 max-w-5xl mx-auto">
        {filtered.map((t, i) => (
          <div key={t.id} style={{ animationDelay: `${i * 60}ms` }} className="animate-[fadeIn_400ms_ease-out_both]">
