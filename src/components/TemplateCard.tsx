...
export default function TemplateCard({ template }: { template: Template }) {
  ...
  return (
    <div className="group rounded-[var(--radius-xl)] bg-[var(--color-surface)] border border-[var(--color-border)] hover:border-[var(--color-border-hover)] transition-ui overflow-hidden">
      <div className="aspect-[4/3] bg-[var(--color-base)] flex items-center justify-center text-5xl relative overflow-hidden">
      ...
      <div className="p-4 pt-3">
        ...
        <h3 className="text-[15px] font-semibold text-[var(--color-text-primary)] mb-0.5">{template.name}</h3>
        ...
        <button onClick={handleUse}
          className="w-full py-2.5 bg-[var(--color-accent)] hover:bg-[var(--color-accent-hover)] text-white rounded-[var(--radius-md)] text-[13px] font-medium transition-ui">
          使用此模板
        </button>
      </div>
    </div>
